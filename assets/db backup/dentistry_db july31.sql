...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 31, 2023 at 02:40 AM
...
CREATE TABLE `taxes` (
  ...
  `delivered_amount` double(12,2) DEFAULT NULL,
  `due_amount` decimal(12,2) DEFAULT NULL,
  `TIN` int(10) DEFAULT NULL,
  `delivery_date` date NOT NULL,
  `tax_for_year` date NOT NULL,
  `delivered_by` int(128) NOT NULL,
  `cli_ID` int(128) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `taxes`
  ...
  ADD KEY `sdetail_ID_fk` (`delivered_by`),
  ADD KEY `cli_ID_fk` (`cli_ID`);
...
--
ALTER TABLE `taxes`
  MODIFY `tax_ID` int(128) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `taxes`
  ADD CONSTRAINT `staff_tax_id_fk` FOREIGN KEY (`delivered_by`) REFERENCES `staff` (`staff_ID`) ON DELETE NO ACTION ON UPDATE CASCADE;
